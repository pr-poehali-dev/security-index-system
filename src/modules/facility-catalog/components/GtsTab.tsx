...
export default function GtsTab() {
  ...
  const facilities = allFacilities.filter(f => f.type === 'gts');
  ...
  const handleDelete = (id: string) => {
    ...
    if (components.length > 0) {
      if (!confirm(`У сооружения есть ${components.length} компонентов. Удалить сооружение и все компоненты?`)) {
        return;
      }
    }
    deleteFacility(id);
    toast({ title: 'ГТС удалено' });
  };
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Всего ГТС</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Гидротехнические сооружения</CardTitle>
