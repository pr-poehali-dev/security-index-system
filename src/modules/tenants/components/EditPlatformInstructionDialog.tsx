...
interface EditPlatformInstructionDialogProps {
  document: KnowledgeDocument;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export default function EditPlatformInstructionDialog({
  document,
  open,
  onOpenChange,
}: EditPlatformInstructionDialogProps) {
  ...
  useEffect(() => {
    if (document) {
      ...
      setContentType(document.content ? 'text' : 'file');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateDocument(
      document.id,
      {
        ...
        tags: tagsArray,
        status,
        publishedAt: status === 'published' && !document.publishedAt 
          ? new Date().toISOString() 
          : document.publishedAt,
      },
      'Обновление инструкции платформы',
      'SuperAdmin'
    );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Редактировать инструкцию платформы</DialogTitle>
        </DialogHeader>
