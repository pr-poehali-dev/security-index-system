...
ObjectAccessManagement.tsx     - Управление доступом к объектам
...
- [ ] Интеграция с модулем Каталог (реальные объекты)
- [ ] Виджет персонала на странице объекта
- [ ] Реальная проверка соответствия по требованиям объекта
...
1. **Mock-объекты** - используются тестовые объекты вместо реальных из модуля Каталог
2. **Упрощенная проверка** - проверка соответствия показывает статичные данные
3. **Без уведомлений** - нет автоматических оповещений об истекающих документах
4. **Без интеграций** - модуль работает изолированно от других модулей

Эти ограничения будут устранены в следующих версиях.
